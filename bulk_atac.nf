...
process filter {
    ...
    input:
        ...
        val FILTER_A from params.filter_f
        val FILTER_B from params.filter_F
        val MAPQ_THRESH from params.mapQ_thr
        val MITO_NAME from params.mito_name
    output:
        path "${OUT_DIR}/noMito_Q${MAPQ_THRESH}_F${FILTER_B}_f${FILTER_A}_FILTERED-${MARKDUP_RAW_BAM_FILE.baseName.tokenize('-').get(1)}.bam" into filteredBam_ch1
        path "${OUT_DIR}/noMito_Q${MAPQ_THRESH}_F${FILTER_B}_f${FILTER_A}_FILTERED-${MARKDUP_RAW_BAM_FILE.baseName.tokenize('-').get(1)}.bam" into filteredBam_ch2
        path "${OUT_DIR}/noMito_Q${MAPQ_THRESH}_F${FILTER_B}_f${FILTER_A}_FILTERED-${MARKDUP_RAW_BAM_FILE.baseName.tokenize('-').get(1)}.bam" into filteredBam_ch3
    shell:
    '''
    samtools index !{MARKDUP_RAW_BAM_FILE} 
    samtools idxstats !{MARKDUP_RAW_BAM_FILE} | cut -f 1 | grep -v !{MITO_NAME} | xargs samtools view -F !{FILTER_B} -f !{FILTER_A} -q !{MAPQ_THRESH} -u !{MARKDUP_RAW_BAM_FILE} | samtools sort /dev/stdin -@ !{task.cpus} -o !{OUT_DIR}/noMito_Q!{MAPQ_THRESH}_F!{FILTER_B}_f!{FILTER_A}_FILTERED-!{MARKDUP_RAW_BAM_FILE.baseName.tokenize('-').get(1)}.bam
    '''

}

process post_filter_QC {
    label 'default'
    input:
        path FILTERED_BAM from filteredBam_ch1
        path QC_DIR from params.qc_dir
        path src from params.src_folder
    shell:
    '''
    samtools index -@ !{task.cpus} !{FILTERED_BAM}
    sample_name=`echo !{FILTERED_BAM.baseName} | awk -F'-' '{print $NF}'`
    Rscript !{src}/ATAC_QC.R \
            -b !{FILTERED_BAM} \
            -o !{QC_DIR} \
            -p post_filter_${sample_name}
    ''' 
}

process post_filter_fingerplot {
    label 'default'
    input:
        path BAM_FILE from filteredBam_ch2.collect()
        path QC_DIR from params.qc_dir
    shell:
    '''
    for b in !{BAM_FILE}
    ...
    plotFingerprint -b !{BAM_FILE} -p !{task.cpus} -plot !{QC_DIR}/post_filter_FingerPrintPlot.pdf
    '''
}
...
process call_peaks_unfiltered {
    label 'default'
    input:
        tuple val(REP), file(BAMS) from markdupRawBam_ch4.map { p -> [p.baseName.tokenize('-').get(1).tokenize('_').get(0), p]}.groupTuple()
        path OUT_DIR from params.out_dir
        val Q_VAL from params.macs_q
        val SHIFT from params.macs_shift
        val GENOME_SIZE from params.macs_effective_genome_size
        val EXTENSION_SIZE from params.macs_extension_size
    output:
        path "${OUT_DIR}/unfiltered_${REP}_q${Q_VAL}_peaks.narrowPeak" into unfilteredBed_ch
    shell:
    '''
    macs2 callpeak -t !{BAMS} \
    -q !{Q_VAL} \
    -n unfiltered_!{REP}_q!{Q_VAL} \
    --outdir !{OUT_DIR} \
    -f BAMPE \
    -g !{GENOME_SIZE} \
    --nomodel \
